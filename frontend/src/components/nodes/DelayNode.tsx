      ...
      <ContextMenuTrigger>
        <div className={`rounded-lg border bg-white px-4 py-3 shadow-sm ${selected ? 'ring-2 ring-blue-500' : ''}`}>
          <div className="flex items-center">
            <div className="mr-3 flex h-10 w-10 items-center justify-center rounded-full bg-violet-100">
              <Clock className="h-5 w-5 text-violet-500" />
            </div>
            <div>
              <div className="font-semibold">{data.label}</div>
              <div className="text-xs text-gray-500">{data.description}</div>
              {data.config?.days !== undefined && (
                <div className="mt-1 text-xs font-mono px-2 py-1 bg-gray-100 rounded">
                  Wait {data.config.days} {data.config.days === 1 ? 'day' : 'days'}
                </div>
              )}
            </div>
          </div>
